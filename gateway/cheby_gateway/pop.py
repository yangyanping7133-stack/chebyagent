...
def canonical_target(raw_path: bytes, raw_query: bytes) -> bytes:
    if not raw_path.startswith(b"/") or b"\n" in raw_path or b"\r" in raw_path:
        raise ProofError("invalid request target")
    if b"\n" in raw_query or b"\r" in raw_query:
        raise ProofError("invalid request target")
    return raw_path + (b"?" + raw_query if raw_query else b"")
...
def canonical_request(
    *,
    ...
    if not method_bytes or any(value in method_bytes for value in (b"\n", b"\r")):
        raise ProofError("invalid method")
    return b"\n".join(
        (
            CANONICAL_PREFIX,
            method_bytes,
